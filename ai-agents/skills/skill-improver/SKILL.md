...
Every such document must be agent-agnostic: it must work under any host agent (e.g. Claude Code, Copilot CLI), so flag and fix any instruction that depends on a single agent's tools or features without a portable fallback.
