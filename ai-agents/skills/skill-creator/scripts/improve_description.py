...
uses the session's Copilot CLI auth, no separate API key needed).
"""
...
def _call_copilot(prompt: str, model: str | None, timeout: int = 300) -> str:
    ...
    `--silent` keeps stdout to the agent response only, which is what the
    caller parses. Stdin piping is unreliable under Copilot, so the prompt
    goes over argv.
    """
    cmd = ["copilot", "-p", prompt, "--silent", "--no-ask-user", "--log-level", "none"]
    if model:
